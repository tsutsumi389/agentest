...
```json
{
  ...
  "data": [
    {
      "date": "2026-01-01",
      "users": { "free": 1000, "pro": 234 },
      "organizations": {
        "team": 70,
        "enterprise": 19,
        "teamMembers": 350,
        "enterpriseMembers": 190
      }
    },
    {
      "date": "2026-01-02",
      "users": { "free": 1010, "pro": 240 },
      "organizations": {
        "team": 72,
        "enterprise": 19,
        "teamMembers": 360,
  ...
  "current": {
    "users": {
      "total": 1250,
      "byPlan": {
        "free": { "count": 1010, "percentage": 80.8 },
        "pro": { "count": 240, "percentage": 19.2 }
      }
    },
    "organizations": {
      "total": 91,
      "totalMembers": 550,
      "byPlan": {
        "team": { "count": 72, "percentage": 79.1, "members": 360 },
        "enterprise": { "count": 19, "percentage": 20.9, "members": 190 }
      }
    },
    "combined": {
      "total": 1800,
      "byPlan": {
        "free": { "count": 1010, "percentage": 56.1 },
        "pro": { "count": 240, "percentage": 13.3 },
        "team": { "count": 360, "percentage": 20.0 },
        "enterprise": { "count": 190, "percentage": 10.6 }
      }
    }
  },
  "fetchedAt": "2026-02-01T10:00:00.000Z"
...
| endDate | string | 終了日時（ISO 8601形式） |
| timezone | string | タイムゾーン |
| view | string | ビュー（combined/users/organizations） |
| data | PlanDistributionDataPoint[] | 時系列データ |
| current | PlanDistributionCurrent | 最新時点のサマリー |
| fetchedAt | string | 取得日時（ISO 8601形式） |
...
|-----------|------|------|
| date | string | 日付（YYYY-MM-DD形式） |
| users | object | ユーザープラン分布（view=users/combined時） |
| users.free | number | FREEプランユーザー数 |
| users.pro | number | PROプランユーザー数 |
| organizations | object | 組織プラン分布（view=organizations/combined時） |
| organizations.team | number | TEAM組織数 |
| organizations.enterprise | number | ENTERPRISE組織数 |
| organizations.teamMembers | number | TEAMプラン組織メンバー総数 |
| organizations.enterpriseMembers | number | ENTERPRISEプラン組織メンバー総数 |
...
| members | number | メンバー数（組織プランの場合のみ） |
...
- **過去データのみ**: TTL 5分
- **当日データ含む**: TTL 1分
- **キャッシュキー**: `admin:metrics:plan-distribution:${hash}`
...
```bash
curl -X GET "http://localhost:3001/api/v1/admin/metrics/plan-distribution?granularity=day&view=combined" \
  -H "Cookie: admin_session=your-session-id"
```
...
const response = await fetch(`/api/v1/admin/metrics/plan-distribution?${params}`, {
  method: 'GET',
  credentials: 'include',
});

const data = await response.json();
console.log(data.current.users.total); // ユーザー総数
console.log(data.current.organizations.byPlan.team.members); // TEAMプラン組織メンバー数
```
