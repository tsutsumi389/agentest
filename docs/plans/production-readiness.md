...
### H-4: ~~分散レートリミッティングの実装~~ → 対応不要（インフラ層で対応）

**結論**: API側のレートリミッティング（`express-rate-limit`）は全削除済み。本番環境ではCloud Load Balancer + Cloud Armorでインフラ層のレート制限を行うため、アプリケーション層での実装は不要。

**本番デプロイ前の前提条件（Cloud Armor設定）**:
- Cloud Armorセキュリティポリシーの作成
- 認証エンドポイント（`/api/auth/*`, `/admin/auth/*`, `/oauth/*`）に対する厳格なレートリミットルール
  - 例: 1分あたり10リクエスト/IPアドレス
- 一般APIエンドポイントに対する標準レートリミットルール
  - 例: 1分あたり100リクエスト/IPアドレス
- DDoS防護ルールの有効化
- ステージング環境での動作確認
...
| APIセキュリティ | ○ | Helmet, CORS, CSRF | レート制限はインフラ層で対応 |
