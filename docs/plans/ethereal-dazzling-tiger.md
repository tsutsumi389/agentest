...
MCP ServerにMCP Apps機能を追加し、テストスイート一覧を表示するインタラクティブUIと、テスト実行をLLMに依頼する機能を実装する。
...
1. `pnpm build`でUIがバンドルされることを確認
2. MCP Inspectorでツールが登録されていることを確認
3. Claude DesktopでUIが表示されることを確認
4. テスト実行ボタンの動作確認
