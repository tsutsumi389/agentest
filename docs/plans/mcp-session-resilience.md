...
| ID | 生成方法 | 寿命 | 用途 |
|---|---------|------|------|
| **マシンID** | `os.hostname()` または `HOSTNAME` 環境変数 | コンテナ/マシンの生存中 | 同一マシン上での再起動検知 |
| **インスタンスID** | `randomUUID()` | プロセス生存中 | プロセス単位の一意識別・生存確認 |

開発環境（`tsx watch`）ではプロセス再起動してもホスト名は変わらないため、マシンIDで「同じマシン上の再起動」を検知できる。
Cloud Run では各コンテナインスタンスが異なるホスト名を持つため、マシンIDで「別インスタンスへのルーティング」を検知できる。
...
リクエスト受信 (インスタンスA)
  ...
  └─ インメモリにセッションなし
       └─ Redis参照
            ...
            ├─ Redisにあり & 同じマシンID → "server_restarted"
            │   （同じマシンだがインスタンスIDが異なる＝プロセス再起動）
            └─ Redisにあり & 別のマシンID
...
また、ホスト名ベースのマシンIDを提供し、同一マシン上でのプロセス再起動を検知可能にする。
...
export async function registerServerInstance(): Promise<void> {
  ...
  await redis.setex(
    ...
    JSON.stringify({ machineId: MACHINE_ID, startedAt: new Date().toISOString() })
  );
}
...
| 関数 | 説明 |
|------|------|
| `saveSession(sessionId, data)` | セッションメタデータ + インスタンスID + マシンIDをRedisに保存 |
...
{ "machineId": "hostname-or-container-id", "startedAt": "2026-02-24T12:00:00.000Z" }

// mcp:session:{sessionId}
{
  "userId": "user-uuid",
  "instanceId": "instance-uuid",
  "machineId": "hostname-or-container-id",
...
| テストファイル | テスト対象 |
|---------------|-----------|
| `__tests__/unit/lib/server-instance.test.ts` (新規) | インスタンスID生成、マシンID取得、Redis登録/延長、生存確認、Redis未設定時のフォールバック |
| `__tests__/unit/lib/session-store.test.ts` (新規) | save/get/delete/refreshTtl、Redis未設定時のフォールバック |
| `__tests__/unit/transport/streamable-http.test.ts` (新規) | `resolveSessionError` の4パターン判定（session_not_found / server_restarted[同一マシンID] / wrong_instance / instance_terminated） |
