...
## Phase 1: 不要ファイルの削除（18ファイル + 3ディレクトリ）
...
### 1-3. 内部実装計画削除（`docs/plans/` 全3ファイル → ディレクトリごと削除）

- `glowing-skipping-harp.md` / `jolly-crunching-pond.md` / `stateful-nibbling-seahorse.md`
- **注意**: 本プランファイル（wild-percolating-lightning.md）もplans/配下だが、実装完了後に削除

### 1-4. その他の削除

- `docs/guides/onboarding.md` — 内部チームオンボーディング（Slack, Notion, 1Password参照）
- `docs/architecture/diagrams/gcp-architecture.drawio.drawio` — GCP固有アーキテクチャ図
- `docs/architecture/diagrams/gcp-architecture.drawio.svg` — 同上のSVG
...
- `docs/plans/` （3ファイル + ディレクトリ）※本プランファイル含む
- `docs/guides/onboarding.md`
- `docs/architecture/diagrams/gcp-architecture.drawio.drawio`
- `docs/architecture/diagrams/gcp-architecture.drawio.svg`
