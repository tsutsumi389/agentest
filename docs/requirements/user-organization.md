...
| MCP リクエスト | 100/月 | 1,000/月 | 5,000/月/ユーザー | 無制限 |
| AI アシスト実行 | 10回/月 | 100回/月 | 500回/月/ユーザー | 無制限 |
| MCP 同時接続数 | 1 | 3 | 5/ユーザー | 無制限 |
| 実行履歴保持期間 | 30日 | 1年 | 1年 | 無制限 |
...
*エビデンスは実行履歴と連動して削除（履歴保持期間を超えたものは自動削除）

### 使用量制限の考え方

- **MCP リクエスト**: MCP サーバー経由での API 呼び出し回数（月次リセット）
- **AI アシスト実行**: AI によるテストケース生成・実行支援の回数（月次リセット）
...
| BIL-007 | 年払いオプション | 年払い選択で割引適用（2ヶ月分無料） |
| BIL-008 | 使用量表示 | 現在の使用量（MCP リクエスト、AI アシスト実行回数）を表示 |
| BIL-009 | 使用量アラート | 月間制限の 80% に達した際のメール通知 |
...
| 使用量アラート | 月間制限（MCP/AI アシスト）の 80% に到達 | ON |
| 課金通知 | 請求・支払い関連 | ON |
...
| 使用量制限 | MCP リクエスト・AI アシスト実行回数は月次でリセット |
...
| `UsageRecord` | 月次使用量記録（MCP リクエスト、AI アシスト実行） |
