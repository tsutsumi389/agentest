...
| テストケース数/プロジェクト | 100 | 無制限 | 無制限 | 無制限 |
| 実行履歴保持期間 | 30日 | 1年 | 1年 | 無制限 |
| エビデンスストレージ | 500MB | 5GB | 10GB/ユーザー | 無制限 |
| MCP 同時接続数 | 1 | 3 | 5/ユーザー | 無制限 |
| API レート制限 | 60/時 | 1,000/時 | 5,000/時 | カスタム |
| 組織メンバー数 | - | - | 無制限 | 無制限 |
| SSO (SAML/OIDC) | - | - | - | ○ |
| 監査ログ | - | - | 90日 | 無制限 |
| 専用サポート | - | - | - | ○ |
...
| ID | 機能 | 説明 |
|----|------|------|
| USR-001 | ユーザー登録 | メールアドレスまたは OAuth でアカウント作成 |
| USR-002 | プロフィール設定 | 表示名、アバター、メールアドレスの設定 |
| USR-003 | OAuth 連携 | GitHub / Google アカウントでログイン・連携 |
| USR-004 | パスワード認証 | メール + パスワードでのログイン（OAuth 未使用時） |
| USR-005 | パスワードリセット | メールによるパスワードリセット |
| USR-006 | メール認証 | アカウント作成時のメールアドレス確認 |
| USR-007 | 二要素認証 | TOTP による二要素認証の設定 |
| USR-008 | セッション管理 | アクティブセッションの確認・無効化 |
| USR-009 | アカウント削除 | 自身のアカウントを削除（30日後に物理削除） |
| USR-010 | 個人プラン選択 | Free / Pro プランの選択・変更 |
...
| BIL-008 | 使用量表示 | 現在の使用量（ストレージ、API コール等）を表示 |
| BIL-009 | 使用量アラート | 制限に近づいた際のメール通知 |
...
| カテゴリ | イベント |
|----------|----------|
| 認証 | ログイン、ログアウト、パスワード変更、2FA 設定変更 |
...
| `Session` | ユーザーセッション |
| `TwoFactorAuth` | 二要素認証設定 |

### 既存テーブルの変更

| テーブル | 変更内容 |
|---------|---------|
| `User` | `plan`, `emailVerified`, `avatarUrl` 追加 |
