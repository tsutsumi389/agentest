...
セッション再起動耐性のため、PostgreSQL の AgentSession に加えて Redis にセッションルーティング情報を保存する。

| Redisキー | TTL | 内容 | 用途 |
|-----------|-----|------|------|
| `mcp:session:{sessionId}` | 180秒 | `userId`, `instanceId`, `machineId`, `createdAt` | セッションがどのインスタンスに存在するか判定 |
| `mcp:instance:{instanceId}` | 120秒 | `machineId`, `startedAt` | インスタンスの生存確認 |

**AgentSession（PostgreSQL）との関係:**

- AgentSession はビジネスロジック（エージェント活動の追跡、操作記録）を担当する永続データ
- Redis のセッションルーティング情報はインフラ層の一時データで、セッション消失時の障害原因判定に使用
- セッション作成時に両方に書き込み、リクエスト毎に Redis の TTL を延長
- Redis データは TTL で自動失効し、グレースフルシャットダウン時にも削除される
...
| AG-012 | セッション再起動耐性 | Redis によるセッションルーティングと障害原因判定 |
