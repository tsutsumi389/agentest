...
| Free | 固定値 | 100 |
| Pro | 固定値 | 1,000 |
| Team | レコード作成時に算出 | 5,000 × 組織メンバー数 |
| Enterprise | 固定値 | 0（無制限） |

**無制限の扱い**: `mcpSessionLimit = 0` は無制限を意味する。アプリケーション層で `limit === 0` の場合は制限チェックをスキップする。

### Team プランの制限更新

Team プランでは月初にバッチ処理で `mcpSessionLimit` を再計算する。

```
月初バッチ処理:
1. 新規 UsageRecord 作成
2. OrganizationMember をカウント
3. mcpSessionLimit = 5,000 × メンバー数 を設定
```
