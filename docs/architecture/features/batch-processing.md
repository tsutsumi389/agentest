...
| `history-expiry-notify` | 毎日 9:00 JST | 履歴削除予告の通知 |
...
### 2. history-expiry-notify（履歴削除予告通知）

削除7日前の FREE ユーザーにメール通知を送信します。
...
### 4. payment-event-cleanup（決済イベントクリーンアップ）
...
### 5. subscription-sync（サブスクリプション同期）
...
| `history-expiry-notify` | 個別ユーザー単位でスキップして継続 |
...
| `SMTP_HOST` | No* | SMTP サーバー（history-expiry-notify で必要） |
