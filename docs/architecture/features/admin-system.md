...
システム管理者（運営者）向けの管理機能を提供する。ユーザー認証とは完全に独立したセッション管理を持ち、システム全体のユーザー管理、システム監視、運用を行うための機能群。
...
| ID | 機能名 | 説明 | 状態 |
|----|--------|------|------|
| ADM-PROF-001 | プロフィール更新 | 表示名を変更 | 実装済 |
| ADM-PROF-002 | パスワード変更 | 現在のパスワード検証付きでパスワード変更、他セッション失効 | 実装済 |
