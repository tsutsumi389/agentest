...
- **形式選択**: CSV / JSON
- **フィルター適用**: 現在適用中のフィルター条件でエクスポート
- **ダウンロード**: ブラウザの標準ダウンロード機能を使用
- **ファイル名**: `audit-logs-{timestamp}.{format}`
- **CSV仕様**
  - BOM付きUTF-8（Excel対応）
  - ヘッダー行: ID, 日時, カテゴリ, アクション, ユーザー, 対象タイプ, 対象ID, IPアドレス, 詳細
- **JSON仕様**
  - 整形済み（pretty print）
  - ユーザー情報を含む
...
sequenceDiagram
    ...
    F->>F: 形式選択ドロップダウン表示
    U->>F: CSV/JSONを選択
    F->>B: GET /api/organizations/{id}/audit-logs/export?format=csv
    B->>B: 権限確認（OWNER/ADMINのみ）
    B->>DB: 監査ログ取得（フィルタ適用）
    B->>B: CSV/JSON形式に変換
    B->>F: ファイルデータ（Content-Disposition: attachment）
    F->>U: ファイルダウンロード開始
```
