...
| MCP-031 | セッション再起動耐性 | Redis によるセッションルーティングと障害検知 | 実装済 |
...
sequenceDiagram
    ...
    Note over M,R: サーバー起動時
    M->>R: registerServerInstance()（インスタンスIDをRedisに登録）
    M->>H: ハートビート監視開始（30秒間隔）

    loop 30秒ごと
        H->>R: refreshInstanceHeartbeat()（インスタンスTTL延長）
    ...
    Note over A,M: 新規セッション作成時
    A->>M: POST /mcp (initialize)
    M->>R: saveSession()（セッションメタデータをRedisに保存）
    M->>DB: AgentSession 作成
    M->>A: レスポンス

    Note over A,M: 通常のリクエスト
    A->>M: POST /mcp (任意のリクエスト)
    M->>R: refreshSessionTtl()（セッションTTL延長）
    M->>DB: lastHeartbeat 更新
    M->>A: レスポンス

    Note over A,M: シャットダウン時
    M->>R: cleanupAllSessions()（全セッションのRedisデータを削除）
```
...
| AgentSession | PostgreSQL | ステータス, ハートビート, プロジェクト紐付け | 永続 | ビジネスロジック。エージェント活動の追跡 |

#### Redisキー設計

| キー | 型 | TTL | 内容 |
|------|-----|-----|------|
| `mcp:instance:{instanceId}` | STRING | 120秒 | インスタンス情報（`machineId`, `startedAt`） |
| `mcp:session:{sessionId}` | STRING | 180秒 | セッションメタデータ（`userId`, `instanceId`, `machineId`, `createdAt`） |

#### インスタンス管理

| ID | 生成方法 | 寿命 | 用途 |
|---|---------|------|------|
| `SERVER_INSTANCE_ID` | `randomUUID()` | プロセス生存中 | プロセス単位の一意識別・生存確認 |
| `MACHINE_ID` | `os.hostname()` / `HOSTNAME` 環境変数 | コンテナ/マシンの生存中 | 同一マシン上での再起動検知 |

開発環境（`tsx watch`）ではプロセス再起動してもホスト名は変わらないため、マシンIDで「同じマシン上の再起動」を検知できる。Cloud Run では各コンテナインスタンスが異なるホスト名を持つため、マシンIDで「別インスタンスへのルーティング」を検知できる。
...
セッションが見つからない場合、Redis のルーティング情報から障害原因を判定し、詳細なエラーレスポンスを返す。
...
全パターンで `data.reinitialize: true` が含まれるため、クライアントは `initialize` から再接続することで復旧可能。
...
| INSTANCE_TTL | 120秒 | インスタンスRedis TTL（ハートビート間隔の4倍） |
| SESSION_REDIS_TTL | 180秒 | セッションRedis TTL（ハートビートタイムアウトの3倍） |
