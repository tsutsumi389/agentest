...
| MCP-027 | CIMDクライアント識別 | HTTPS URL `client_id` からメタデータ自動取得（事前登録不要） | 実装済 |
...
| MCP-025 | エビデンスアップロードツール | 期待結果にエビデンスをアップロード（Presigned URL方式、3ステップ） | 実装済 |
| MCP-030 | エビデンスアップロード確認ツール | Presigned URLアップロードの完了確認 | 実装済 |
| MCP-031 | MCPセッション管理UI | Settings画面からMCPセッション一覧確認・終了 | 実装済 |
...
| メソッド | パス | 説明 | 認証 |
|----------|------|------|------|
| POST | /internal/api/executions/:executionId/expected-results/:expectedResultId/evidences?userId=xxx | エビデンスアップロード（multipart形式、Web UI用） | 共有シークレット |
| POST | /internal/api/executions/:executionId/expected-results/:expectedResultId/evidences/upload-url?userId=xxx | Presigned URL生成 | 共有シークレット |
| POST | /internal/api/executions/:executionId/evidences/:evidenceId/confirm?userId=xxx | アップロード確認 | 共有シークレット |
...
MCP クライアント（Claude Code 等）向けの OAuth 2.1 認証フロー。PKCE、動的クライアント登録（DCR）、CIMD、リソースインジケーターに対応。

#### クライアント識別の経路分岐

`client_id` の形状でクライアント識別経路を自動分岐する。

| 経路 | client_id 形式 | 識別方法 |
|------|----------------|----------|
| **CIMD**（MCP 2025-11 デフォルト） | HTTPS URL（例: `https://example.com/mcp/client.json`） | URL から JSON メタデータを取得 |
| **DCR**（既存互換） | UUID | 事前に `POST /oauth/register` で登録 |
...
sequenceDiagram
    ...
    participant Doc as CIMD Document Host
    participant B as Browser
    ...
    A->>C: AS Metadata (client_id_metadata_document_supported: true)
    Note over C: 事前登録なしで CIMD URL を client_id として使用
    C->>B: Open authorize URL (client_id=https://...)
    B->>A: GET /oauth/authorize
    A->>A: client_id 形状判定 (CIMD URL)
    alt キャッシュ鮮度内
        A->>A: DB から既存メタデータを使用
    else 期限切れ or 未登録
        A->>Doc: GET <client_id> [If-None-Match: <etag>]
        Doc->>A: 200 OK (JSON) or 304 Not Modified
        A->>A: SSRF 検証 / メタデータ検証 / upsert
    end
    A->>B: Login/Consent screen
    U->>B: Approve
    B->>A: POST /oauth/authorize/consent
    A->>B: Redirect with code
    B->>C: code (via redirect_uri)
...
| メソッド | パス | 説明 | 認証 |
|----------|------|------|------|
| GET | `/.well-known/oauth-authorization-server` | Authorization Server Metadata（CIMD 広告含む） | 不要 |
| POST | `/oauth/register` | 動的クライアント登録（DCR 経路のみ） | 不要 |
| GET | `/oauth/authorize` | 認可エンドポイント（CIMD/DCR を内部分岐） | 必須 |
| POST | `/oauth/authorize/consent` | 同意承認 | 必須 |
| POST | `/oauth/token` | トークン発行（CIMD/DCR を内部分岐） | 不要 |
| POST | `/oauth/introspect` | トークン検証 | 内部API |
| POST | `/oauth/revoke` | トークン失効 | 不要 |

### CIMDクライアント識別

MCP仕様（2025-11更新）でデフォルトのクライアント識別メカニズムに採用された CIMD（Client ID Metadata Document, draft-ietf-oauth-client-id-metadata-document-00）に対応。

#### 構成

CIMD 関連の実装は Authorization Server（`apps/api`）側に集約されている。MCP サーバー（`apps/mcp-server`）は Protected Resource として変更不要。

```
apps/api/src/
├── services/
│   ├── oauth.service.ts              # resolveClient() / AS Metadata
│   └── cimd/
│       ├── cimd-url.ts               # CIMD URL 判定 + URL要件検証（純粋関数）
│       ├── cimd-fetcher.ts           # HTTP fetch + ETag + Cache-Control 解釈
│       ├── cimd-validator.ts         # メタデータJSONのzod検証 + CIMD固有ルール
│       └── cimd-service.ts           # 上記を束ねてupsertまで（in-flight キャッシュ）
├── repositories/
│   └── oauth.repository.ts           # upsertCimdClient / touchCimdClient
├── utils/
│   └── safe-fetch.ts                 # SSRF対策fetchラッパ
└── config/env.ts                     # CIMD_MAX_BYTES / CIMD_FETCH_TIMEOUT_MS / CIMD_CACHE_TTL_SEC
```
...
| 項目 | 内容 |
|------|------|
| **経路分岐** | `client_id` が HTTPS URL → CIMD、UUID → DCR を内部で自動判別 |
| **キャッシュ** | DB（`oauth_clients`）に upsert。`metadataExpiresAt` で鮮度判定 |
| **条件付きリクエスト** | `ETag` を保存、期限切れ時は `If-None-Match` で 304 Not Modified を期待 |
| **Cache-Control 解釈** | `max-age=N` / `no-store` / 既定値（`CIMD_CACHE_TTL_SEC=3600`）を解釈 |
| **並行フェッチ抑止** | プロセス内 in-flight Promise マップ + DB upsert で冪等化 |
| **フォールバック** | フェッチ失敗時、既存有効キャッシュがあればスタールキャッシュを返す（warn ログ） |

#### SSRF対策（`utils/safe-fetch.ts`）

CIMD URL のフェッチには、以下の SSRF 対策を施した HTTP クライアントを使用する。

1. URL 検証：`https:` 必須、fragment / userinfo 禁止
2. DNS 解決した全 IP を検証し、以下を拒否：
   - `private` / `loopback` / `linkLocal` / `uniqueLocal` / `multicast` / `broadcast` / `reserved` / `unspecified`
   - `169.254.0.0/16`（AWS / GCP メタデータサービス）
   - `100.64.0.0/10`（CGNAT）
3. 検証済み IP に直接接続（`servername` で SNI、`Host` ヘッダはホスト名）→ DNS rebinding 対策
4. リダイレクトは最大 3 ホップ手動追跡（各ホップで SSRF 再検査）
5. レスポンスサイズ上限（既定 5KB）超過で abort
6. `Content-Type` が `application/json` で始まることを確認
7. タイムアウト（既定 5 秒）

#### CIMD 固有バリデーション（`cimd-validator.ts`）

| ルール | 内容 |
|---|---|
| `client_id` 一致 | メタデータJSON内 `client_id` プロパティとフェッチ URL の完全一致（仕様必須） |
| 対称鍵プロパティ拒否 | `client_secret` プロパティを含む場合は拒否 |
| 対称鍵 auth method 拒否 | `client_secret_basic` / `client_secret_post` / `client_secret_jwt` は拒否 |
| `redirect_uris` 必須 | 配列・1 件以上・URL 形式 |
| `grant_types` 制限 | `authorization_code` / `refresh_token` のみ受理 |
| `response_types` 制限 | `code` のみ受理 |

#### redirect_uri 検証の経路分岐

| 経路 | 検証ルール |
|---|---|
| **DCR** | `localhost` / `127.0.0.1` のみ許可（`POST /oauth/register` 時に検証） |
| **CIMD** | メタデータ宣言の `redirect_uris` 配列と完全一致（任意ドメイン可） |

CIMD 経路では DB に保存された `redirect_uris`（メタデータ宣言値）と入力値を比較するため、authorize エンドポイントの `redirect_uri` 検証ロジックは経路を意識しない。
...
| `CIMD_CACHE_TTL_SEC` | `3600` | `Cache-Control` ヘッダがない場合の既定 TTL（秒） |

#### MCP サーバー側の変更

`apps/mcp-server/src/routes/oauth-metadata.ts` の Protected Resource Metadata は変更不要。
`authorization_servers` フィールドが AS の URL を指しており、クライアントはその URL から AS Metadata を引いて `client_id_metadata_document_supported: true` を確認する。
...
- クライアント識別は2経路をサポート（`client_id`の形状で自動分岐）
  - **CIMD**（draft-ietf-oauth-client-id-metadata-document-00）: HTTPS URL `client_id` から JSON メタデータを取得（事前登録不要）
  - **DCR**（RFC 7591）: UUID `client_id` を `POST /oauth/register` で事前登録
...
- redirect_uri 検証（経路別）:
  ...
  - CIMD: メタデータ宣言の `redirect_uris` 集合と完全一致のみ許可（任意ドメイン可）
...
| ツール名 | 説明 | 状態 |
|----------|------|------|
| upload_execution_evidence | Presigned URL取得（3ステップアップロードのStep 1） | 実装済 |
| confirm_evidence_upload | アップロード確認（3ステップアップロードのStep 3） | 実装済 |
