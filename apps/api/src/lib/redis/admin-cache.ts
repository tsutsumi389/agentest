...
export async function setAdminUsersCache<T>(
  ...
  return setCache(
    ...
    data,
    ttlSeconds,
    '管理者ユーザー一覧キャッシュの保存に失敗'
  );
}

export async function getAdminUsersCache<T>(params: Record<string, unknown>): Promise<T | null> {
  return getCache<T>(
    generateParamsKey(KEY_PREFIX.ADMIN_USERS, params),
...
export async function getAdminUserDetailCache<T>(userId: string): Promise<T | null> {
  return getCache<T>(
    `${KEY_PREFIX.ADMIN_USER_DETAIL}${userId}`,
    '管理者ユーザー詳細キャッシュの取得に失敗'
  );
}
...
export async function invalidateAdminOrganizationsCache(): Promise<boolean> {
  return invalidateCacheByPattern(
    `${KEY_PREFIX.ADMIN_ORGANIZATIONS}*`,
    '管理者組織一覧キャッシュの無効化に失敗'
  );
}
...
export async function getAdminOrganizationDetailCache<T>(
  ...
  return getCache<T>(
    `${KEY_PREFIX.ADMIN_ORGANIZATION_DETAIL}${organizationId}`,
    '管理者組織詳細キャッシュの取得に失敗'
  );
}
