...
describe('AgentSessionService', () => {
  ...
  describe('endSession', () => {
    ...
    it('存在しないセッションの場合NotFoundErrorを投げる', async () => {
      ...
      await expect(service.endSession(TEST_USER_ID, 'nonexistent-id')).rejects.toThrow(
        NotFoundError
      );
    });
    ...
    it('OAuthトークンを失効させる', async () => {
      ...
      mockAgentSessionRepo.revokeOAuthToken.mockResolvedValue({
        ...mockToken,
        revokedAt: new Date(),
      });
    ...
    it('他人のOAuthトークンは失効できない', async () => {
      ...
      await expect(service.endSession(TEST_USER_ID, 'oauth-token-1')).rejects.toThrow(
        AuthorizationError
      );
    });
    ...
    it('プロジェクトメンバーでない場合AuthorizationErrorを投げる', async () => {
      ...
      await expect(service.endSession(TEST_USER_ID, TEST_SESSION_ID)).rejects.toThrow(
        AuthorizationError
      );
    });
