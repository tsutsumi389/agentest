...
describe('requireTestCaseRole', () => {
  ...
  describe('認証チェック', () => {
    it('req.userがない場合はAuthenticationErrorを投げる', async () => {
      ...
      const middleware = requireTestCaseRole(['READ']);
  ...
  describe('パラメータチェック', () => {
    it('testCaseIdがない場合はAuthorizationErrorを投げる', async () => {
      ...
      const error = (mockNext as ReturnType<typeof vi.fn>).mock.calls[0][0];
      expect(error.message).toBe('Test case ID required');
    });

    it('無効なUUID形式のtestCaseIdでBadRequestErrorを投げる', async () => {
      const req = createMockRequest({ params: { testCaseId: 'invalid-uuid' } });
      const res = createMockResponse();
      ...
      const error = (mockNext as ReturnType<typeof vi.fn>).mock.calls[0][0];
      expect(error.message).toBe('Invalid test case ID format');
    });
  });

  describe('テストケース存在チェック', () => {
    ...
    it('削除済みテストケースはNotFoundErrorを投げる（allowDeletedTestCase: false）', async () => {
    ...
    it('削除済みテストケースでもallowDeletedTestCase: trueなら通過する', async () => {
      mockPrisma.testCase.findUnique.mockResolvedValue({
        id: TEST_CASE_ID,
        deletedAt: new Date(),
        testSuite: {
      ...
      const middleware = requireTestCaseRole(['READ'], { allowDeletedTestCase: true });
      await middleware(req as Request, res as Response, mockNext);
  ...
  describe('プロジェクトオーナー権限', () => {
    it('プロジェクトオーナーは全権限を持つ', async () => {
      ...
      const middleware = requireTestCaseRole(['ADMIN']); // ADMIN権限が必要
      await middleware(req as Request, res as Response, mockNext);
  ...
  describe('プロジェクトメンバー権限', () => {
    it('必要なロールを持つメンバーは通過する', async () => {
      ...
      const middleware = requireTestCaseRole(['WRITE', 'ADMIN']);
      await middleware(req as Request, res as Response, mockNext);
    ...
    it('必要なロールを持たないメンバーはAuthorizationErrorを投げる', async () => {
      ...
      const middleware = requireTestCaseRole(['ADMIN']); // ADMIN必須
      await middleware(req as Request, res as Response, mockNext);
    ...
    it('VIEWERロール（READ）で書き込み操作を拒否', async () => {
      ...
      const middleware = requireTestCaseRole(['OWNER', 'ADMIN', 'WRITE']); // 書き込み権限
      await middleware(req as Request, res as Response, mockNext);
  ...
  describe('組織メンバー権限フォールバック', () => {
    it('プロジェクトメンバーでなくても組織OWNER/ADMINなら通過する', async () => {
      ...
      const middleware = requireTestCaseRole(['WRITE']);
      await middleware(req as Request, res as Response, mockNext);
  ...
  describe('複数ロールの許可', () => {
    it.each([
      ...
      const middleware = requireTestCaseRole(allowedRoles as any);
      await middleware(req as Request, res as Response, mockNext);
  ...
  describe('エラーハンドリング', () => {
    it('prismaエラーはnextに渡される', async () => {
      ...
      mockPrisma.testCase.findUnique.mockRejectedValue(dbError);
