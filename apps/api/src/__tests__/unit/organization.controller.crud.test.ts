...
describe('OrganizationController（CRUD・メンバー・招待・プロジェクト・オーナー権限）', () => {
  ...
  describe('invite', () => {
    it('メンバーを招待して201を返す', async () => {
      ...
      mockOrgService.invite.mockResolvedValue({ invitation: mockInvitation, emailSent: false });
      ...
      expect(res.status).toHaveBeenCalledWith(201);
      expect(res.json).toHaveBeenCalledWith({ invitation: mockInvitation, emailSent: false });
    });
