...
export function UserDetail() {
  ...
  const { data, isLoading, isFetching, error, refetch } = useAdminUserDetail(id ?? '');
    ...
    <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-8">
      ...
      {error && !isLoading && (
        <div className="bg-error/10 border border-error/20 rounded-lg p-6 text-center">
          <p className="text-error font-medium mb-2">ユーザー情報の取得に失敗しました</p>
      ...
      {data && !isLoading && (
        <div className="space-y-6">
          ...
          <div className="bg-background-secondary border border-border rounded-lg p-4">
            <h2 className="text-sm font-medium text-foreground-muted mb-4">基本情報</h2>
            <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
              <div className="flex items-center gap-3">
                <Calendar className="w-5 h-5 text-foreground-muted" />
                <div>
                  <p className="text-sm text-foreground-muted">作成日</p>
                  <p className="text-sm text-foreground">{formatDateTime(data.user.createdAt)}</p>
                </div>
              </div>
              <div className="flex items-center gap-3">
                ...
                <div>
                  ...
                  <p className="text-sm text-foreground">{formatDateTime(data.user.updatedAt)}</p>
                </div>
              </div>
              {data.user.deletedAt && (
                <div className="flex items-center gap-3">
                  ...
                  <div>
                    ...
                    <p className="text-sm text-error">{formatDateTime(data.user.deletedAt)}</p>
                  </div>
                </div>
              )}
            </div>
          ...
          {/* 組織・OAuth */}
          <div className="space-y-6">
            <UserOrganizationsSection organizations={data.user.organizations} />
