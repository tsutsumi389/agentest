...
echo ""
echo "=== API サーバー ==="
docker build --platform linux/amd64 -t "${REGISTRY}/api:${TAG}" -f docker/Dockerfile.api .
docker push "${REGISTRY}/api:${TAG}"

echo ""
echo "=== WebSocket サーバー ==="
docker build --platform linux/amd64 -t "${REGISTRY}/ws:${TAG}" -f docker/Dockerfile.ws .
docker push "${REGISTRY}/ws:${TAG}"

echo ""
echo "=== MCP サーバー ==="
docker build --platform linux/amd64 -t "${REGISTRY}/mcp:${TAG}" -f docker/Dockerfile.mcp .
docker push "${REGISTRY}/mcp:${TAG}"

echo ""
echo "=== Web フロントエンド ==="
docker build --platform linux/amd64 -t "${REGISTRY}/web:${TAG}" -f docker/Dockerfile.web \
...
echo ""
echo "=== Admin フロントエンド ==="
docker build --platform linux/amd64 -t "${REGISTRY}/admin:${TAG}" -f docker/Dockerfile.admin \
  --build-arg VITE_API_URL="${VITE_API_URL}" .
docker push "${REGISTRY}/admin:${TAG}"

echo ""
echo "=== バッチジョブ ==="
docker build --platform linux/amd64 -t "${REGISTRY}/jobs:${TAG}" -f apps/jobs/Dockerfile .
